...
export function BespokeSolutionsIntro({
  ...
  // subtle scroll progress for the whole section
    ...
    <section className="relative bg-background text-foreground overflow-hidden">
      ...
      <div className="section-container">
        <motion.div variants={stagger} initial="hidden" whileInView="show" viewport={{ once: true, amount: 0.25 }}>
          {/* Pill + heading */}
          <motion.span variants={fadeUp} className="inline-flex items-center gap-2 rounded-full border border-foreground/15 bg-foreground/5 px-3 py-1 text-xs font-semibold">
            <Sparkles className="h-4 w-4" /> Bespoke Solutions
          </motion.span>

          <motion.h2 variants={fadeUp} className="relative mt-3 text-4xl md:text-5xl font-extrabold tracking-tight">
            ...
            <motion.span
              ...
              className="block h-1 w-36 md:w-48 bg-gradient-to-r from-primary to-emerald-500 rounded-full mt-3 origin-left"
            />
          </motion.h2>

          <motion.p variants={fadeUp} className="mt-5 text-base md:text-lg text-muted-foreground max-w-prose">
          ...
          {/* Points grid with hover-lift and glow */}
          <motion.ul variants={stagger} className="mt-8 grid gap-4 sm:grid-cols-2">
            {POINTS.map((p, i) => (
              <motion.li
                ...
                variants={fadeUp}
                whileHover={{ y: -3 }}
                className="group relative flex items-start gap-3 rounded-xl border border-foreground/10 bg-white/60 dark:bg-neutral-900/60 backdrop-blur px-4 py-3 shadow-sm"
              >
...
/* =========================================================
   SECTION 2: Our Development Process (Cards w/ effects)
========================================================= */
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {STEPS.map((s, idx) => (
            <motion.article
              ...
              className="relative group rounded-2xl border border-foreground/10 bg-white/60 dark:bg-neutral-900/60 backdrop-blur p-5 md:p-6 shadow-[0_10px_30px_-15px_rgba(0,0,0,0.25)]"
            >
              {/* glossy gradient border on hover */}
              ...
              {/* step badge */}
              <div className="absolute -left-1.5 -top-3 grid size-8 place-items-center rounded-full bg-background ring-2 ring-foreground/10 shadow">
